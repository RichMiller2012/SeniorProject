...
namespace DAO.Data
{
    public class TransactionsDAOImpl : AbstractBaseDAO
    {
        ...
        public int saveTransactionData(Transactions trans, int storeId, int custId)
        {
            using (ISession session = NHibernateHelper.OpenSession())
            {
                using (ITransaction transaction = session.BeginTransaction())
                {
                    if (custId != 0)
                    {
                        Customer customer = session.Get<Customer>(custId);
                        customer.transactions.Add(trans);
                    }

                    Store store = session.Get<Store>(storeId);
                    store.transactions.Add(trans);

                    transaction.Commit();

                    return (int)session.GetIdentifier(trans);
                }
            }
        }
