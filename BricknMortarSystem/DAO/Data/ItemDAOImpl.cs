...
namespace DAO.Data
{
    public class ItemDAOImpl : AbstractBaseDAO
    {
        ...
        public Barcode getBarcodeById(int barcodeId)
        {
            using (ISession session = NHibernateHelper.OpenSession())
            {
                using (ITransaction transaction = session.BeginTransaction())
                {
                    var item = session.QueryOver<Barcode>()
                        .Where(id => id.barcodeId == barcodeId)
                        .SingleOrDefault();

                    return new Barcode((Barcode)item);
                }
            }
        }
        ...
        public PartNo getPartNoById(int partNoId)
        {
            using (ISession session = NHibernateHelper.OpenSession())
            {
                using (ITransaction transaction = session.BeginTransaction())
                {
                    var item = session.QueryOver<PartNo>()
                        .Where(id => id.partNoId == partNoId)
                        .SingleOrDefault();

                    return new PartNo((PartNo)item);
                }
            }
        }
